...
def CreateExcel(MA,num_years,filename,num_shares,margin):
    ...
    def Find_Trades(df,MA):
        ...
        column_close = df.columns.get_loc('Close')
        rows = len(df.axes[0])
        LorS = ['Neither']
    ...
    def Trades_df(df,MA,num_shares,margin):
        ...
        trades_df = pd.DataFrame(columns = ['Entry Date','Entry Price','Exit Date','Exit Price',str(MA[0])+'MA',
                                            str(MA[1])+'MA','Profit','Long/Short','ROI'])
        for i in range(len(trades.axes[0])-1,1,-1):
            ...
            MA5 = trades.iloc[i,trades.columns.get_loc(str(MA[0])+'MA')]
            MA13 = trades.iloc[i,trades.columns.get_loc(str(MA[1])+'MA')]
            ...
            row = {'Entry Date':entry_date,'Entry Price':entry_price,'Exit Date':exit_date,'Exit Price':exit_price,
                   str(MA[0])+'MA':MA5,str(MA[1])+'MA':MA13,'Profit':profit,'Long/Short':long_short,'ROI':ROI}
            trades_df = trades_df.append(row,ignore_index=True)

        #print(trades.tail(50))
        #excel_table.to_excel('MA_Backtest_Result.xlsx',index=False)
        return trades_df
...
#Creates another sheet giving Expectancy ratios in descending order.
...
def CreateCombinations():
    fast_MA = list(range(3,10)) #Possible short term MA
    slow_MA = list(range(12,26)) #Possible long term MA
    combinations = []

    for i in range(len(fast_MA)):
        for j in range(len(slow_MA)):
            combinations.append([fast_MA[i],slow_MA[j]])
    return combinations

#Creates excel sheets
def CreateExcelSheets(filename,combinations,num_years,num_shares,margin):
    excel_sheets = []
    for i in range(len(combinations)):
        trades_df = CreateExcel(combinations[i],num_years,filename,num_shares,margin)
        excel_sheets.append(trades_df)
    return excel_sheets

#Creates Excel doc
def CreateExcelDoc(excel_sheets,stock_name,combinations):
    doc_name = '2MA_backtest_result_'+str(stock_name)+'.xlsx'
    writer = pd.ExcelWriter(doc_name, engine='xlsxwriter')
    for i in range(len(excel_sheets)):
        sheet_name = 'MA-'+str(combinations[i][0])+','+str(combinations[i][1])
...
def TwoMAStrategy(stock_name,filename,num_years,num_shares,margin):
    combinations = CreateCombinations()
    excel_sheets = CreateExcelSheets(filename,combinations,num_years,num_shares,margin)
    CreateExcelDoc(excel_sheets,stock_name,combinations)
